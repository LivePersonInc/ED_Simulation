 ...
 * @author bmathijs
 */
public class ED_Simulation_TimeVarying {
    ...
    public ED_Simulation_TimeVarying(double[] lambda, double cycleLength, double mu, double delta, int[] s, int[] n, double p) {
    ...
    public SimResults_T simulate(double maxTime) {
        ...
        while (t < maxTime) {
            ...
            results.registerQueueLengths(holdingQueue.getSize(), serviceQueue.getSize(), contentQueue.getSize(),s[interval],n[interval],t,interval);

            if (e.getType() == Event.ARRIVAL) {
                Patient newPatient = new Patient(t,interval);
                ...
                int addedBeds = newN - oldN;
                
                for( int j = 0; j < addedBeds; j++ ){
                    if ( holdingQueue.getSize() > 0 ){
                        Patient pt = holdingQueue.removeFirstPatient();
                        results.registerHoldingTime(pt, t);
                        pt.setLastArrivalTime(t);
                    }
                }
...
8, 8, 8, 9, 8, 8, 8, 7};
        ...
        for( int i = 0; i < s.length; i++ ) nn[i] = n;
        
        ED_Simulation_TimeVarying sim = new ED_Simulation_TimeVarying(l, cycleLength, mu, delta, s, nn, p);
        SimResults_T results = sim.simulate(100000);
        
        System.out.println("Mean hold :\t "+ results.getTotalMeanHoldingTime());
        System.out.println("Mean hold c:\t "+ results.getTotalMeanHoldingTimeConditional());
        System.out.println("Mean inner wait:\t "+ results.getTotalMeanWaitingTime());
        System.out.println("Mean inner wait c:\t "+ results.getTotalMeanWaitingTimeConditional());
